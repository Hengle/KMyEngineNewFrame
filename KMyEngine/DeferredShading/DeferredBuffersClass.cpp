...
bool DeferredBuffersClass::OnResize(ID3D11Device* pd,int width,int height)
{
	...
	for(i=0; i<BUFFER_COUNT; i++)
	{
		HR(pd->CreateTexture2D(&textureDesc, NULL, &m_renderTargetTextureArray[i]));
	}
	...
	for(i=0; i<BUFFER_COUNT; i++)
	{
		HR(pd->CreateRenderTargetView(m_renderTargetTextureArray[i], &renderTargetViewDesc, &m_renderTargetViewArray[i]));
	}
	...
	for(i=0; i<BUFFER_COUNT; i++)
	{
		HR(pd->CreateShaderResourceView(m_renderTargetTextureArray[i], &shaderResourceViewDesc, &m_shaderResourceViewArray[i]));
	...
	HR(pd->CreateTexture2D(&depthBufferDesc, NULL, &m_depthStencilBuffer));
	...
	HR(pd->CreateDepthStencilView(m_depthStencilBuffer, &depthStencilViewDesc, &m_depthStencilView));
